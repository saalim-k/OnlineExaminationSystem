...
<h3> ALL currently registered Admins </h3>
...
{
	echo 	'<tr>
				<td class="columnRegistered">'.$Arr2['User_ID'].'</td>
				<td class="columnRegistered">'.$Arr2['F_Name'].'</td>
				<td class="columnRegistered">'.$Arr2['L_Name'].'</td>
				<td class="columnRegistered">'.$Arr2['DOB'].'</td>
				<td class="columnRegistered">'.$Arr2['Phone_Num'].'</td>
				<td class="columnRegistered">'.$Arr2['Email_Add'].'</td>
			</tr>';
}
?>
</table>
...
{
	echo "<h2> A new Admin was registred successfully.<h2>";
...
if(isset($_POST['submit']))
{
	...
	else
	{
		...
		$InsertPeopleQuery="INSERT INTO People(User_ID,Acc_type,F_Name,L_Name,DOB,Phone_Num,Email_Add)VALUES('$Uid','A','$Fn','$Ln','$dob','$Pn','$Eadd')";
		$insert1=mysqli_query($connect,$InsertAccQuery);
		$insert2=mysqli_query($connect,$InsertPeopleQuery);
		if($insert1){
			if($insert2){
					header("location:RegNewAdmin.php?Usern=$User&Passn=$Pass");
				}
			}
		else{
			echo "<h3>Username or User ID already exists, please try another username or User ID</h3>";
		}
	}
}
?>
<br>
<h3>Enter the Details of the new Admin Below</h3>
<table class="tableR">
<tr>
